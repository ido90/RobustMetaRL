...
class HumanoidBodyEnv(HumanoidEnv):
    ...
    def set_task(self, task):
        ...
        for i in range(len(self.model.actuator_acc0)):
            self.model.actuator_acc0[i] = task[0] * self.original_acc[i]
        for i in range(len(self.model.body_mass)):
            self.model.body_mass[i] = task[1] * self.original_mass[i]
        for i in range(len(self.model.geom_size)):
            self.model.geom_size[i, 0] = task[1] * self.original_size[i, 0]
        # for i in range(len(self.model.geom_size)):
        #     self.model.geom_size[i, 1] = task[2] * self.original_size[i, 1]

        # mass   / butt, rfoot, rhand
        # length / butt, rfoot, rhand
        # for i_task, (i_size, i_mass) in enumerate(zip((5, 8, 14), (3, 6, 11))):
        #     # mass + width
        #     self.model.body_mass[i_mass] = task[i_task] * self.original_mass_vec[i_mass]
        #     self.model.geom_size[i_size, 0] = task[i_task] * self.original_size_vec[i_size, 0]
        #     # length
        #     self.model.geom_size[i_size, 1] = task[i_task+3] * self.original_size_vec[i_size, 1]
