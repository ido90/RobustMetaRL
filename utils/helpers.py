...
def reset_env(env, args, indices=None, state=None, cem=None, return_weights=False):
    ...
    if (indices is None) or (len(indices) == args.num_processes):
        ...
        if cem is not None:
            tasks, weights = zip(*[cem.sample() for _ in range(args.num_processes)])
            env.set_task(tasks)
    ...
    if return_weights and weights is not None:
        return state, belief, task, weights
